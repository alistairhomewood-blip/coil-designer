...
function basePath(c,N=200){
  ...
  if(c.type === "circular"){
    ...
    const L = c.straightLength/2, R = c.arcRadius, seg = Math.max(8, N/4|0);
  ...
  for(let lay=0; lay<layers && count<coil.turns; lay++){
    ...
    for(let j=0; j<n; j++){
      ...
      if(isTor){
        ...
        const bp = basePath(coil,128);
        const cen = new THREE.Vector3();
        bp.forEach(p=>cen.add(p));
        cen.divideScalar(bp.length || 1);
        const up = new THREE.Vector3(0,1,0);
        const op = bp.map(p=>{
          const rd = p.clone().sub(cen);
          if(rd.length() > 1e-12) rd.normalize();
...
function resampleClosedPath(path,nSegs){
  ...
  if(total <= 0) return pts.map(p=>p.clone());
  const out = [];
  for(let i=0;i<nSegs;i++){
    const s = (i/nSegs) * total;
    let k = 0;
    while(k < pts.length-1 && cum[k+1] < s) k++;
    const segLen = Math.max(cum[k+1] - cum[k], 1e-12);
    const t = (s - cum[k]) / segLen;
    out.push(pts[k].clone().lerp(pts[(k+1)%pts.length], t));
  }
  return out;
}

function sampleTurnIndices(total,samples){
  if(total <= 0) return [];
  if(samples >= total) return [...Array(total)].map((_,i)=>i);
  const out = [];
  for(let i=0;i<samples;i++) out.push(Math.min(total-1, Math.round((i+0.5)*total/samples - 0.5)));
  return [...new Set(out)];
}

/* ══════════════════════════════════════════════════════════════
   FIELD MODEL + BIOT-SAVART
   ══════════════════════════════════════════════════════════════ */

function buildFieldModel(coils,{segmentsPerLoop=96,turnSamples=4}={}){
  const mx=[], my=[], mz=[], dlx=[], dly=[], dlz=[], coeff=[];

  const addLoop = (path, ampTurns) => {
    const p = resampleClosedPath(path, Math.max(12, segmentsPerLoop|0));
    const c0 = MU0 * ampTurns / (4*Math.PI);
  ...
  coils.forEach(c=>{
    const ts = Math.max(1, turnSamples|0);
    if(c.turns > 1 && ts > 1){
      const allTurns = turnPaths(c);
      const idxs = sampleTurnIndices(allTurns.length, Math.min(ts, allTurns.length));
      const rep = idxs.length > 0 ? allTurns.length / idxs.length : 1;
      idxs.forEach(idx=>addLoop(allTurns[idx], c.current * rep));
    }else{
      addLoop(fullPath(c, Math.max(64, segmentsPerLoop)), c.current * c.turns);
    }
  });
  ...
  return {
    ...
    coeff:Float64Array.from(coeff), count:coeff.length,
    segmentsPerLoop, turnSamples
  };
}

function biotSavartFromModel(model,pt){
  if(!model || !model.count) return new THREE.Vector3();
  let Bx=0, By=0, Bz=0;
  const px=pt.x, py=pt.y, pz=pt.z;
  const {mx,my,mz,dlx,dly,dlz,coeff,count} = model;
  for(let i=0;i<count;i++){
    const rx = px - mx[i], ry = py - my[i], rz = pz - mz[i];
...
function getTorusParams(coils){
  ...
  const c = tor[0];
  return {
    R:c.majorRadius, r:c.minorRadius, ext:c.type === "elongated_toroidal_winding" ? (c.extension||0) : 0,
    center:new THREE.Vector3(...c.center),
    normal:new THREE.Vector3(...c.normal).normalize().length() > 1e-12 ? new THREE.Vector3(...c.normal).normalize() : new THREE.Vector3(0,1,0)
  };
}
...
function isInsideTorus(pt,tp){
  ...
  if(tp.ext <= 0) return distFromTube*distFromTube + y*y < tp.r*tp.r;
  if(y > tp.ext/2) return distFromTube*distFromTube + (y-tp.ext/2)*(y-tp.ext/2) < tp.r*tp.r;
  if(y < -tp.ext/2) return distFromTube*distFromTube + (y+tp.ext/2)*(y+tp.ext/2) < tp.r*tp.r;
  return Math.abs(distFromTube) < tp.r;
}

function sampleTorusCrossLocal(tp){
  for(let tries=0;tries<2000;tries++){
    const dft = (Math.random()*2-1) * tp.r * 0.98;
    const y = (Math.random()-0.5) * (tp.ext + 2*tp.r) * 0.98;
    if(tp.ext <= 0){
      if(dft*dft + y*y < tp.r*tp.r) return {dft,y};
    }else{
      if(y > tp.ext/2){ if(dft*dft + (y-tp.ext/2)*(y-tp.ext/2) < tp.r*tp.r) return {dft,y}; }
      else if(y < -tp.ext/2){ if(dft*dft + (y+tp.ext/2)*(y+tp.ext/2) < tp.r*tp.r) return {dft,y}; }
      else if(Math.abs(dft) < tp.r) return {dft,y};
    }
  }
  return {dft:0,y:0};
}

function torusLocalToWorld(tp,rho,y,theta){
  const local = new THREE.Vector3(rho*Math.cos(theta), y, rho*Math.sin(theta));
  const q = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0,1,0), tp.normal);
  return local.applyQuaternion(q).add(tp.center);
}

function sampleTorusBodyPoint(tp){
  const {dft,y} = sampleTorusCrossLocal(tp);
  const theta = Math.random()*2*Math.PI;
  return torusLocalToWorld(tp, tp.R + dft, y, theta);
}

function sampleHolePoint(tp){
  const holeR = Math.max(0.01, tp.R - tp.r*0.9);
  const totalH = tp.ext + 2*tp.r;
  const rho = holeR * Math.sqrt(Math.random()) * 0.95;
  const theta = Math.random() * 2 * Math.PI;
  const y = (Math.random()-0.5) * totalH * 0.7;
  return torusLocalToWorld(tp, rho, y, theta);
}
...
function avgBInside(model,coils,nSamples=200){
  ...
  const samples = [];
  for(let i=0;i<nSamples;i++) samples.push(sampleTorusBodyPoint(tp));
  let sum=0;
  samples.forEach(p=>{ sum += biotSavartFromModel(model,p).length(); });
  return { avg:sum/Math.max(1,samples.length), n:samples.length };
}
...
function simulateElectrons(model,coils,opts){
  const tp = getTorusParams(coils);
  if(!tp || !model || !model.count) return [];
  const count = Math.max(1, opts.count|0);
  const eMin = Math.max(1, opts.energyMinEV || 1e4);
  const eMax = Math.max(eMin, opts.energyMaxEV || 1e6);
  const launchDistance = Math.max(0.1, opts.launchDistance || 10);
  const aimMode = opts.aimMode || "mixed";
  const maxSteps = Math.max(200, opts.maxSteps || 1600);
  const totalScale = tp.R + tp.r + tp.ext/2;
  const baseDs = Math.max(0.005, Math.min(0.05, launchDistance/350, Math.max(0.02, totalScale/25)));
  const out = [];

  for(let i=0;i<count;i++){
    const u = count === 1 ? 0 : i/(count-1);
    const logE = Math.log10(eMin) + (Math.log10(eMax)-Math.log10(eMin))*u;
    const energyEV = 10**logE;
    const gamma = 1 + (energyEV * E_CHARGE) / (E_MASS * C0 * C0);
    const speed = C0 * Math.sqrt(1 - 1/(gamma*gamma));

    const target = aimMode === "torus"
      ? sampleTorusBodyPoint(tp)
      : aimMode === "hole"
        ? sampleHolePoint(tp)
        : (Math.random() < 0.5 ? sampleTorusBodyPoint(tp) : sampleHolePoint(tp));

    const sourceDir = new THREE.Vector3().randomDirection();
    const pos = tp.center.clone().add(sourceDir.multiplyScalar(launchDistance));
    const vel = target.clone().sub(pos).normalize().multiplyScalar(speed);
    const path = [pos.clone()];
    let entered = isInsideTorus(pos,tp);
    let minDist = pos.distanceTo(tp.center);

    for(let step=0; step<maxSteps; step++){
      const B = biotSavartFromModel(model,pos);
      const Bmag = B.length();
      let dt = baseDs / speed;
      if(Bmag > 1e-12){
        const gyroDt = 0.15 * gamma * E_MASS / (E_CHARGE * Bmag);
        dt = Math.min(dt, Math.max(1e-13, gyroDt));
      }
      if(Bmag > 1e-12){
        const axis = B.clone().normalize();
        const angle = -(E_CHARGE * Bmag * dt) / (gamma * E_MASS);
        vel.applyAxisAngle(axis, angle);
      }
      pos.add(vel.clone().multiplyScalar(dt));
      if(step % 2 === 0) path.push(pos.clone());
      if(isInsideTorus(pos,tp)) entered = true;

      const dist = pos.distanceTo(tp.center);
      if(dist < minDist) minDist = dist;
      if(step > 20 && dist > launchDistance*1.15 && dist > minDist + Math.max(0.5, totalScale*1.5)) break;
      if(dist > launchDistance*1.5) break;
    }

    out.push({ path, entered, energyEV });
  }
  return out;
}
...
function tubeMesh(path,tubeR,color,emissive){
  ...
  const m = new THREE.MeshStandardMaterial({
    color:new THREE.Color(color), emissive:new THREE.Color(emissive ? color : "#000"), emissiveIntensity:emissive ? 0.5 : 0,
...
function ghostTorus(R,r,center,normal){
  ...
  const m = new THREE.MeshStandardMaterial({color:0xffffff,transparent:true,opacity:0.07,side:THREE.DoubleSide,depthWrite:false});
...
function ghostElongTorus(R,r,ext,center,normal){
  const pts = [];
  for(let i=0;i<=64;i++){
    const {dr,dh} = stadiumPt(i/64,r,ext);
...
function buildEnclosedVolume(R,r,ext,center,normal){
  ...
  if(ext > 0){
    ...
    g = new THREE.TorusGeometry(R,r,32,100);
  }
...
function makeLabel(text,pos,color){
  ...
  s.position.copy(pos); s.scale.set(0.06,0.06,0.06);
  return s;
}

/* ══════════════════════════════════════════════════════════════
   FIELD LINES + PARTICLE MESHES
   ══════════════════════════════════════════════════════════════ */
...
function traceFieldLine(model,start,steps,ds){
  ...
  for(let i=0;i<steps;i++){
    const B = biotSavartFromModel(model,p);
    const mag = B.length();
    pts.push(p.clone()); mags.push(mag);
    if(mag < 1e-15) break;
    const dir = B.clone().normalize();
    if(prevDir && dir.dot(prevDir) < -0.2) break;
    const adapt = baseDs * Math.min(1, 0.001/Math.max(mag,1e-10));
    const stepSize = Math.max(baseDs*0.12, Math.min(baseDs*2, adapt));
...
function generateFieldLines(model,density){
  ...
  heights.forEach(y=>seedDists.forEach(r=>{
    for(let i=0;i<nAround;i++){
      ...
      const fwd=traceFieldLine(model,seed,400,0.005); if(fwd.pts.length>5) lines.push(fwd);
      const bwd=traceFieldLine(model,seed,400,-0.005); if(bwd.pts.length>5) lines.push(bwd);
    }
  }));
  return lines;
}
...
function buildParticleMeshes(trajs){
  const group = new THREE.Group();
  trajs.forEach(tr=>{
    if(!tr.path || tr.path.length < 2) return;
    const geom = new THREE.BufferGeometry().setFromPoints(tr.path);
    const mat = new THREE.LineBasicMaterial({color:tr.entered ? 0xef4444 : 0x22c55e, transparent:true, opacity:0.95});
...
function CurrentEditor({coils,onCommitCoils,highlightId,setHighlightId}){
  const wrapRef = useRef(null);
  const canvasRef = useRef(null);
  const crossRef = useRef(null);
  const dragRef = useRef({active:false,index:-1,draft:null});
  ...
  useEffect(()=>{
    ...
    if(!el) return;
    const update = ()=>setWidth(Math.max(320, Math.floor(el.clientWidth - 8)));
    update();
  ...
  const viewCoils = dragRef.current.active && dragRef.current.draft ? dragRef.current.draft : coils;
  const maxI = useMemo(()=>Math.max(1,...viewCoils.map(c=>Math.abs(c.current)))*1.3,[viewCoils]);
  const dpr = typeof window !== "undefined" ? (window.devicePixelRatio || 1) : 1;
  const CW = width, CH = 190, XW = width, XH = 160;
  ...
  useEffect(()=>{
    ...
    if(viewCoils.length === 0) return;

    const startX = 40, endX = CW-10, zeroY = CH-22, scaleH = zeroY-16;
    const barW = Math.max(5, Math.min(28, ((endX-startX)-viewCoils.length*2)/Math.max(1,viewCoils.length)));
    const gap = 2;

    ctx.strokeStyle="#444"; ctx.lineWidth=1;
    ctx.beginPath(); ctx.moveTo(startX,10); ctx.lineTo(startX,zeroY); ctx.lineTo(endX,zeroY); ctx.stroke();

    ctx.fillStyle="#666"; ctx.font="10px monospace"; ctx.textAlign="right";
    const yStep = Math.max(1, Math.ceil(maxI/6));
    for(let a=0;a<=maxI+1e-9;a+=yStep){
      const y = zeroY - (a/maxI)*scaleH;
      ctx.fillText(a.toFixed(0)+"A", startX-4, y+3);
      ctx.strokeStyle="#1f2937"; ctx.beginPath(); ctx.moveTo(startX,y); ctx.lineTo(endX,y); ctx.stroke();
    }

    viewCoils.forEach((c,i)=>{
      const x = startX+4 + i*(barW+gap);
  ...
  },[viewCoils,highlightId,maxI,width,dpr]);
  ...
  useEffect(()=>{
    ...
    ctx.fillStyle="#0b1220"; ctx.fillRect(0,0,XW,XH);

    const tor = viewCoils.filter(c=>c.type === "toroidal_winding" || c.type === "elongated_toroidal_winding");
    if(tor.length === 0){
    ...
    const c0 = tor[0], R = c0.majorRadius, r = c0.minorRadius, ext = c0.type === "elongated_toroidal_winding" ? (c0.extension||0) : 0;
    const totalH = 2*r + (ext>0 ? ext : 0);
    const totalW = 2*(R+r);
    const scale = Math.min((XW-40)/Math.max(1e-6,totalW),(XH-30)/Math.max(1e-6,totalH))*0.8;
    const cx = XW/2, cy = XH/2;

    ctx.strokeStyle="#444"; ctx.lineWidth=1;
    const outerX = cx + R*scale, innerX = cx - R*scale;
    ...
    tor.forEach(c=>{
      ...
      const isHL = c.id === highlightId;
      const col = COLORS[viewCoils.indexOf(c)%COLORS.length];
      const dotR = isHL ? 5 : 3;
      [px1,px2].forEach(px=>{
  ...
  },[viewCoils,highlightId,width,dpr]);

  const barGeom = useMemo(()=>{
    const startX = 40, endX = width-10;
    const barW = Math.max(5, Math.min(28, ((endX-startX)-viewCoils.length*2)/Math.max(1,viewCoils.length)));
    return {startX,endX,barW,gap:2,zeroY:CH-22,scaleH:(CH-22)-16};
  },[viewCoils.length,width]);

  const getBarIndex = e => {
    const cv = canvasRef.current; if(!cv || viewCoils.length===0) return -1;
    const rect = cv.getBoundingClientRect();
    const x = (e.clientX-rect.left) * (CW/rect.width);
    const {startX,barW,gap} = barGeom;
    return Math.floor((x-(startX+4))/(barW+gap));
  };
  ...
  const getBarCurrent = e => {
    ...
    const {zeroY,scaleH} = barGeom;
    return Math.max(0, Math.round(((zeroY-y)/scaleH)*maxI*10)/10);
  };

  const updateDraft = e => {
    const idx = dragRef.current.index;
    if(idx < 0 || idx >= viewCoils.length) return;
    const base = dragRef.current.draft ? [...dragRef.current.draft] : [...coils];
    base[idx] = {...base[idx], current:Math.min(getBarCurrent(e), maxI*2)};
    dragRef.current.draft = base;
    forceTick(n=>n+1);
  };

  const handleDown = e => {
    const idx = getBarIndex(e);
    if(idx>=0 && idx<viewCoils.length){
      dragRef.current = {active:true,index:idx,draft:[...coils]};
      setHighlightId(viewCoils[idx].id);
      updateDraft(e);
    }
  };

  const handleMove = e => {
    if(dragRef.current.active){ updateDraft(e); return; }
    const idx = getBarIndex(e);
    if(idx>=0 && idx<viewCoils.length) setHighlightId(viewCoils[idx].id);
  };

  useEffect(()=>{
    const onUp = () => {
      if(dragRef.current.active && dragRef.current.draft){
        onCommitCoils(dragRef.current.draft);
      }
      dragRef.current = {active:false,index:-1,draft:null};
    };
  ...
  return (
    <div ref={wrapRef} className="flex flex-col gap-2 w-full">
      ...
      <canvas ref={canvasRef}
        ...
        onMouseLeave={()=>{ if(!dragRef.current.active) setHighlightId(null); }}
      />
      ...
      <GradientTool coils={coils} onUpdateCoils={onCommitCoils}/>
    </div>
  );
}
...
function GradientTool({coils,onUpdateCoils}){
  ...
  const apply = () => {
    ...
    radii.forEach(({id,cR})=>{
      ...
      idMap.set(id, Math.round((innerI + (outerI-innerI)*t)*10)/10);
    });
...
function FieldAccuracyPanel({segmentsPerLoop,setSegmentsPerLoop,turnSamples,setTurnSamples,model,fieldBuildMs}){
  return (
    <div className="border-t border-gray-800 px-3 py-2 bg-gray-900/30 flex flex-col gap-1">
      ...
      <label className="flex items-center gap-2 text-xs text-gray-400">
        <span className="w-28 text-gray-500">Segments/loop</span>
        <select value={segmentsPerLoop} onChange={e=>setSegmentsPerLoop(parseInt(e.target.value))}
          className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none">
          {[24,32,48,64,96,128,160,192].map(v=><option key={v} value={v}>{v}</option>)}
        </select>
      </label>
      <label className="flex items-center gap-2 text-xs text-gray-400">
        <span className="w-28 text-gray-500">Sampled turns</span>
        <select value={turnSamples} onChange={e=>setTurnSamples(parseInt(e.target.value))}
          className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none">
          {[1,2,4,6,8,12,16,24].map(v=><option key={v} value={v}>{v}</option>)}
        </select>
      </label>
      <div className="text-[11px] text-gray-600">
        Higher settings improve Biot–Savart accuracy but slow field lines, interior averages, and particle trajectories.
      </div>
      {model && <div className="text-[11px] text-gray-600">Model: {model.count.toLocaleString()} segments {fieldBuildMs!==null && `· built in ${fieldBuildMs.toFixed(0)} ms`}</div>}
    </div>
  );
}

function ParticlePanel({cfg,setCfg,onRun,onClear,showParticles,setShowParticles,particlesLoading,particleCount}){
  ...
  return (
    <div className="border-t border-gray-800 px-3 py-2 bg-gray-900/30 flex flex-col gap-1.5">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs">
        <label className="text-gray-500">Count
          <input type="number" step={1} value={cfg.count} onChange={e=>set("count", Math.max(1, parseInt(e.target.value)||1))}
            className="w-full mt-0.5 bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none"/>
        </label>
        <label className="text-gray-500">Launch dist (m)
          <input type="number" step={0.1} value={cfg.launchDistance} onChange={e=>set("launchDistance", Math.max(0.1, parseFloat(e.target.value)||0.1))}
            className="w-full mt-0.5 bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none"/>
        </label>
        <label className="text-gray-500">Min energy (eV)
          <input type="number" step={1000} value={cfg.energyMinEV} onChange={e=>set("energyMinEV", Math.max(1, parseFloat(e.target.value)||1))}
            className="w-full mt-0.5 bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none"/>
        </label>
        <label className="text-gray-500">Max energy (eV)
          <input type="number" step={1000} value={cfg.energyMaxEV} onChange={e=>set("energyMaxEV", Math.max(1, parseFloat(e.target.value)||1))}
            className="w-full mt-0.5 bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none"/>
        </label>
      </div>
      <label className="flex items-center gap-2 text-xs text-gray-400">
        <span className="w-28 text-gray-500">Aim mode</span>
        <select value={cfg.aimMode} onChange={e=>set("aimMode",e.target.value)}
          className="bg-gray-800 border border-gray-700 rounded px-1.5 py-0.5 text-gray-200 text-xs outline-none">
          <option value="torus">Aim at torus body</option>
          <option value="hole">Aim at hole</option>
      ...
      <label className="flex items-center gap-2 text-xs text-gray-400 cursor-pointer">
        ...
        Show particle trajectories {particleCount>0 && <span className="text-gray-600">({particleCount})</span>}
      </label>
      <div className="flex gap-2">
        <button onClick={onRun} disabled={particlesLoading}
          className="px-3 py-1 bg-blue-600 hover:bg-blue-500 disabled:bg-gray-700 disabled:text-gray-500 text-white text-xs rounded">
          {particlesLoading ? "Running..." : "Run electrons"}
        </button>
        <button onClick={onClear} className="px-3 py-1 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs rounded">Clear</button>
      </div>
      <div className="text-[11px] text-gray-600">Green = never enters torus habitat volume. Red = enters torus habitat volume. Magnetic-only trajectories.</div>
    </div>
  );
}

function InfoBox({coils,show,setShow,avgBInside:avgBVal,computingAvgB,model,particleStats}){
  ...
  const bO = model ? biotSavartFromModel(model,new THREE.Vector3(0,0,0)) : new THREE.Vector3();
  const b30 = model ? biotSavartFromModel(model,new THREE.Vector3(0.3,0,0)) : new THREE.Vector3();
  return (
    <div className="absolute bottom-3 right-3 bg-gray-900/95 border border-gray-700 rounded-lg p-3 text-xs text-gray-300 z-10 min-w-56 max-w-80">
      ...
      <div className="flex flex-col gap-1">
        ...
        {avgBVal!==null && <div>Avg |B| torus habitat: <span className="text-green-300">{fmtB(avgBVal.avg)}</span> <span className="text-gray-600">({avgBVal.n} pts)</span></div>}
        {computingAvgB && <div className="text-yellow-500">Computing habitat avg...</div>}
        {particleStats && <>
          <div className="border-t border-gray-800 pt-1 mt-1"/>
          <div>Electrons entering habitat: <span className="text-red-300">{particleStats.entered}</span> / {particleStats.total}</div>
        </>}
        <div className="text-gray-600 mt-1 italic">Biot-Savart model: {model ? `${model.count.toLocaleString()} segs` : "n/a"}</div>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const [turnSamples,setTurnSamples] = useState(4);
  const [fieldBuildMs,setFieldBuildMs] = useState(null);
  const [particlesLoading,setParticlesLoading] = useState(false);
  const [particleTrajs,setParticleTrajs] = useState([]);
  const [particleCfg,setParticleCfg] = useState({count:100,launchDistance:10,energyMinEV:1e4,energyMaxEV:1e6,aimMode:"mixed",maxSteps:1600});
  ...
  const fieldModel = useMemo(()=>{
    ...
    const model = buildFieldModel(coils,{segmentsPerLoop,turnSamples});
    model.buildMs = performance.now()-t0;
    return model;
  },[coils,segmentsPerLoop,turnSamples]);

  useEffect(()=>{
    setFieldBuildMs(fieldModel ? fieldModel.buildMs : null);
  },[fieldModel]);

  const particleStats = useMemo(()=>{
    if(!particleTrajs.length) return null;
    const entered = particleTrajs.filter(t=>t.entered).length;
    return {entered,total:particleTrajs.length};
  },[particleTrajs]);
  ...
  useEffect(()=>{
    ...
    const camera = new THREE.PerspectiveCamera(50, el.clientWidth/el.clientHeight, 0.001, 200); cameraRef.current = camera;
    ...
    const d = new THREE.DirectionalLight(0xffffff,0.8); d.position.set(3,5,4); scene.add(d);
    scene.add(new THREE.HemisphereLight(0x4488ff,0x222244,0.3));
    scene.add(new THREE.GridHelper(2,40,0x222222,0x161625));
    scene.add(new THREE.AxesHelper(0.5));
    scene.add(makeLabel("X",new THREE.Vector3(0.55,0,0),"#ef4444"));
    scene.add(makeLabel("Y",new THREE.Vector3(0,0.55,0),"#22c55e"));
    scene.add(makeLabel("Z",new THREE.Vector3(0,0,0.55),"#3b82f6"));
  ...
  useEffect(()=>{
    ...
    if(showArrows) coils.forEach((c,i)=>{
      try{ const arrows=buildCurrentArrows(c,COLORS[i%COLORS.length]); arrows.children.forEach(a=>ag.add(a)); }
      catch(e){ console.warn(e); }
    });
  ...
  useEffect(()=>{
    ...
    if(showVolume){
      const tp = getTorusParams(coils);
      if(tp){
        try{ vg.add(buildEnclosedVolume(tp.R,tp.r,tp.ext,[tp.center.x,tp.center.y,tp.center.z],[tp.normal.x,tp.normal.y,tp.normal.z])); }
        catch(e){ console.warn(e); }
      }
    }
  ...
  useEffect(()=>{
    ...
    if(showField && fieldModel && coils.length>0){
  ...
  },[showField,fieldModel,fieldDensity,coils.length]);
  ...
  useEffect(()=>{
    ...
    if(particleTrajs.length){
      buildParticleMeshes(particleTrajs).children.forEach(c=>pg.add(c));
    }
    pg.visible = showParticles;
  },[particleTrajs,showParticles]);
  ...
  const runParticles = useCallback(()=>{
    if(!fieldModel || !coils.length) return;
    setParticlesLoading(true);
    setTimeout(()=>{
      const trajs = simulateElectrons(fieldModel,coils,particleCfg);
      setParticleTrajs(trajs);
      setShowParticles(true);
      setParticlesLoading(false);
    },50);
  },[fieldModel,coils,particleCfg]);

  const fitView = useCallback(()=>{
    if(coils.length === 0) return;
    const box = new THREE.Box3();
    coils.forEach(c=>fullPath(c,200).forEach(p=>box.expandByPoint(p)));
    const center = new THREE.Vector3(); box.getCenter(center);
    const size = box.getSize(new THREE.Vector3()).length();
    const o = orbitRef.current; o.target.copy(center); o.dist = Math.max(size*1.2,0.1);
    const cam = cameraRef.current; if(!cam) return;
    cam.position.set(
      o.target.x + o.dist*Math.sin(o.phi)*Math.cos(o.theta),
      o.target.y + o.dist*Math.cos(o.phi),
          ...
          <div className="border-t border-gray-800 px-3 py-2 bg-gray-900/30 flex flex-col gap-1">
            ...
            {showVolume && (
              <button onClick={computeAvgB} disabled={computingAvgB}
                ...
                {computingAvgB?"Computing...":"Compute avg |B| in habitat"}
              </button>
            )}
          </div>

          <FieldAccuracyPanel
            segmentsPerLoop={segmentsPerLoop} setSegmentsPerLoop={setSegmentsPerLoop}
            turnSamples={turnSamples} setTurnSamples={setTurnSamples}
            model={fieldModel} fieldBuildMs={fieldBuildMs}
          />

          <ParticlePanel
            cfg={particleCfg} setCfg={setParticleCfg}
            onRun={runParticles} onClear={()=>setParticleTrajs([])}
            showParticles={showParticles} setShowParticles={setShowParticles}
            particlesLoading={particlesLoading} particleCount={particleTrajs.length}
          />
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex-1 relative">
            ...
            <InfoBox coils={coils} show={showStats} setShow={setShowStats}
              avgBInside={avgBVal} computingAvgB={computingAvgB} model={fieldModel} particleStats={particleStats}/>
          ...
          {showCurrentEditor && coils.length>0 && (
            <div className="h-[420px] border-t border-gray-800 bg-gray-950 overflow-y-auto p-3">
              <CurrentEditor coils={coils} onCommitCoils={pushCoils}
